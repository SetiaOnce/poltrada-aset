...
class PemeriksaanAsetController extends Controller
{
    public function index()
    {
        ...
        $data['header_title'] = 'PEMERIKSAAN ASET';
        return view('backend.pemeriksaan', $data);
    }
    ...
    public function data(Request $request)
    {
        ...
        $data = CekDataAset::orderBy('id', 'DESC')->where('jenis_cek_data_aset', 1)->whereBetween('tanggal', [$tgl_start, $tgl_end])->get();
        ...
        if($request->input('fid_jenis')){
            ...
            ->where('aset_cek_data_aset.jenis_cek_data_aset', 1)
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'fid_aset' => 'required',
            'tanggal_pemeriksaan' => 'required|date_format:d/m/Y',
            'judul_kegiatan' => 'required|max:255',
            'keterangan_pemeriksaan' => 'required',
            'foto_pemeriksaan' => 'required|mimes:jpg,jpeg,png|max:2048',
            'file_pdf' => 'required|mimes:pdf|max:10048',
        ],[
            'fid_aset.required' => 'Data aset tidak boleh kosong.',
            'tanggal_pemeriksaan.max' => 'Tanggal pemeriksaan tidak boleh kosong.',
            'tanggal_pemeriksaan.date_format' => 'Tanggal pemeriksaan harus format tanggal (dd/mm/yyyy).',

            'judul_kegiatan.required' => 'Judul kegiatan tidak boleh kosong.',
            'judul_kegiatan.max' => 'Judul kegiatan lebih dari 255 karakter.',
            'keterangan_pemeriksaan.required' => 'Keterangan pemeriksaan tidak boleh kosong.',

            'foto_pemeriksaan.required' => 'Foto pemeriksaan masih kosong.',
            'foto_pemeriksaan.max' => 'Foto pemeriksaan tidak lebih dari 2MB.',
            'foto_pemeriksaan.mimes' => 'Foto pemeriksaan berekstensi jpg, png, jpeg.',
        ...
        if($validator->fails()){
            ...
            // save image pemeriksaan
            $mainImage = $request->file('foto_pemeriksaan');
            $imagename = Shortcut::random_strings(20) . '-pemeriksaan.' . $mainImage->extension();
            Image::make($mainImage)->save(public_path('dist/img/pemeriksaan/'.$imagename));
            // save file pemeriksaan
            $mainfile = $request->file('file_pdf');
            $filename = Shortcut::random_strings(20) . '-pemeriksaan.' . $mainfile->extension();
            $mainfile->move('dist/pdf/', $filename);

            // parse date input
            $newDate=Carbon::createFromFormat('d/m/Y', $request->input('tanggal_pemeriksaan'))->format('Y-m-d'); 

            CekDataAset::create([
                'jenis_cek_data_aset' => 1, //pemeriksaan
    ...
    public function edit(Request $request)
    {
        ...
        $response = array(
            ...
            'urlFoto' => url('dist/img/pemeriksaan/'.$detail->foto_kegiatan),
    ...
    public function update(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            if($request->file('foto_pemeriksaan')){
                $validator = Validator::make($request->all(), [
                    'foto_pemeriksaan' => 'required|mimes:jpg,jpeg,png|max:2048',
                ],[
                    'foto_pemeriksaan.required' => 'Foto pemeriksaan masih kosong.',
