...
/*************************
    OnChange Aset for Form peneriksaan Aset
*************************/
...
const _loadDataPemeriksaan = () => {
    table = $('#dt-pemeriksaanAset').DataTable({
        buttons: [{
            extend: 'print',
                exportOptions: {
                    columns: [0, 1, 2, 3, 4, 5]
        ...
        serverSide: true,
        ajax: {
            url: BASE_URL+ '/ajax/load_pemeriksaan_aset',
        ...
        fnDrawCallback: function (settings, display) {
            ...
            //Custom Table
            $('#dt-pemeriksaanAset_length select').addClass('form-control form-control-sm').selectpicker()
    ...
    $('#dt-pemeriksaanAset').css('width', '100%').DataTable().columns.adjust().draw();
...
function reload_dtpemeriksaanAset(){
	$('#dt-pemeriksaanAset').DataTable().ajax.reload(), $('#dt-pemeriksaanAset').css('width', '100%').DataTable().columns.adjust().draw();
}
// ===>>  SIMULATOR PACKAGE <<===//
...
//Get Detail Data Aset for Form peneriksaan Aset
...
function _detail_aset(idp){
    ...
    //Ajax Load data from ajax
    $.ajax({
        url : BASE_URL+ "/ajax/load_detail_pemeriksaan",
        type: "GET",
        dataType: "JSON",
        data: {
            idp
        },success: function(data)
        {
            if(data.status==true){
                ...
                <div class="alert alert alert-success d-block fadeIn mb-10" role="alert">
                    <div class="row justify-content-center">
                        <div class="col-lg-12">
                            <h3 class="font-weight-normal">Detail Pemeriksaan Aset</h3>
                        </div>
                        <div class="col-lg-6">
                            <div class="d-flex flex-column flex-root mb-3">
                            ...
                            <div class="d-flex flex-column flex-root mb-3">
                                <span class="fs-6 text-gray-400">Tanggal Pemeriksaan: </span>
...
const _clearForm = () => {
    if (save_method == "" || save_method == "add_data") {
        $('#form-data')[0].reset(), $('[name="id"]').val(''), $('[name="methodform_pemeriksaanAset"]').val(''), $("#fid_aset").val('').trigger('change'),$("#tanggal_pemeriksaan").flatpickr({defaultDate: $("#tanggal_pemeriksaan").val(),dateFormat: "d/m/Y"}), $('#keterangan_pemeriksaan').summernote('code', ''),_loadDropifyFile('', '#foto_pemeriksaan'),_loadDropifyFile('', '#file_pdf');
    } else {
        let id = $('[name="id"]').val();
        _editData(id);
    }
}
//Button Reset Form peneriksaan Aset
...
const _addData = () => {
    ...
    $("#card-form .card-header .card-title").html(
        `<h3 class="fw-bolder fs-2 text-gray-900"><i class="bi bi-window-plus fs-2 text-gray-900 me-2"></i>Form Tambah Pemeriksaan Aset</h3>`
    ),
...
const _editData = (idp) => {
    ...
    let target = document.querySelector("#card-form"), blockUi = new KTBlockUI(target, { message: messageBlockUi });
    blockUi.block(), blockUi.destroy();
    load_formaddons(), $('#form-data')[0].reset(), $('[name="methodform_pemeriksaanAset"]').val('update'), $("#fid_aset").val('').trigger('change'), $("#tanggal_pemeriksaan").flatpickr({defaultDate: $("#tanggal_pemeriksaan").val(),dateFormat: "d/m/Y"}),  $('#keterangan_pemeriksaan').summernote('code', ''), _loadDropifyFile('', '#foto_pemeriksaan'),_loadDropifyFile('', '#file_pdf');
    
    //Ajax load from ajax
    $.ajax({
        url: BASE_URL+ '/ajax/pemeriksaan_aset_edit',
        headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
        type: 'GET',
        dataType: 'JSON',
        data: {
        ...
        success: function (data) {
            ...
            if(data.status==true){
                ...
                $('#judul_kegiatan').val(data.judul_kegiatan),$("#tanggal_pemeriksaan").flatpickr({defaultDate: data.tglkegiatan,dateFormat: "d/m/Y"}),_loadDropifyFile(data.urlFoto, '#foto_pemeriksaan'), _loadDropifyFile(data.filepdf, '#file_pdf');
                //Summernote Keterangan
                var keterangan = data.keterangan;
                $('#keterangan_pemeriksaan').summernote('code', keterangan),
                $('#card-form .card-header .card-title').html('<h3 class="card-label"><i class="la la-edit fs-2 text-gray-900 me-2"></i> Form Edit Data Pemeriksaan Aset</h3>');
                $("#card-data").hide(), $("#card-form").show();
            }else{
                swal.fire("Ooops!", "Terjadi kesalahan periksa koneksi internet lalu coba kembali!", "error");
            }
...
$("#btn-save").on("click", function (e) {
    ...
    fotopemeriksaan_preview = $('#fg-fotoPemeriksaan .dropify-preview .dropify-render').html(),
    file_pdf = $('#file_pdf'),
    ...
    if (tanggal_pemeriksaan.val() == '') {
        toastr.error('Tgl. pemeriksaan aset masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        tanggal_pemeriksaan.focus();
        $('#btn-save').html('<i class="far fa-save"></i> Simpan').attr('disabled', false);
        return false;
    }
    if (judul_kegiatan.val() == '') {
        toastr.error('Judul pemeriksaan aset masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        judul_kegiatan.focus();
        $('#btn-save').html('<i class="far fa-save"></i> Simpan').attr('disabled', false);
        return false;
    }
    if (keterangan_pemeriksaan.summernote('isEmpty')) {
        toastr.error('Keterangan pemeriksaan aset masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        keterangan_pemeriksaan.summernote('focus');
        $('#btn-save').html('<i class="far fa-save"></i> Simpan').attr('disabled', false);
        return false;
    }
    if (fotopemeriksaan_preview == '') {
        toastr.error('Foto pemeriksaan aset masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        $('#fg-fotoPemeriksaan .dropify-wrapper').addClass('border-2 border-danger').stop().delay(2500).queue(function () {
    ...
    if (filepdf_priview == '') {
        toastr.error('File pdf pemeriksaan aset masih kosong...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector("#card-form"), blockUi = new KTBlockUI(target, { message: messageBlockUi });
            blockUi.block(), blockUi.destroy();
            let formData = new FormData($("#form-data")[0]), ajax_url = BASE_URL+ "/ajax/pemeriksaan_aset_save";
            if(save_method == 'update_data') {
                ajax_url = BASE_URL+ "/ajax/pemeriksaan_aset_update";
            }
            $.ajax({
                url: ajax_url,
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        var message = 'Data pemeriksaan aset berhasil perbarui'
                        if(save_method == 'add_data'){
                            var message = 'Data pemeriksaan aset baru berhasil ditambahkan'
                        }
                        Swal.fire({
                            title: "Success!",
                            text: message,
                            ...
                            _closeCard('form_pemeriksaan'), _loadDataPemeriksaan();
                        });
                    } else {
